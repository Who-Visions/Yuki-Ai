...
# PURPLE BRAND MODEL - BIKINI BATCH (FAST TRACK)
# Strategy: Universal Placeholder + Strong Prompt Injection
PLACEHOLDER_URL = "https://cdn.myanimelist.net/images/characters/15/315153.jpg"

CHARACTERS = [
    {"name": "Nami", "anime": "One Piece", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Marin Kitagawa", "anime": "My Dress-Up Darling", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Asuna Yuuki", "anime": "Sword Art Online", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Yor Forger", "anime": "Spy x Family", "type": "url", "url": PLACEHOLDER_URL},
]

SUBJECT_NAME = "Purple Brand Model"
SUBJECT_DIR = Path("C:/Yuki_Local/Cosplay_Lab/Subjects/Purple Brand Model")
OUTPUT_DIR = Path("C:/Yuki_Local/Renders_Purple_Bikini_Batch")
...
async def process_character(pipeline, session, mal_client, char_info, bypass_lock, semaphore):
    async with semaphore:
        ...
        # Pattern: v12_5_purple_brand_model_as_{char_name_sanitized}...
        ...
        try:
            ref_path = Path(f"temp_purple_bikini_{sanitized_name}.jpg")
            
            if await get_image_source(session, char_info, mal_client, ref_path):
                # BIKINI THEME PROMPT INJECTION
                target = (
                    f"{char_info['name']} from {char_info['anime']} (Live Action Cosplay, Hyper-Realistic), "
                    "wearing bikini, swimsuit, summer vibes, beach background, cleavage, tanned skin, 8k"
                )
...
async def run_batch():
    ...
    print(f"--- Starting Purple Brand Model (Bikini Batch) ---")
