...
# NINI - BLACK WAIFU BATCH (FAST TRACK)
# Strategy: Universal Placeholder + Strong Prompt Injection for body type and skin tone
PLACEHOLDER_URL = "https://cdn.myanimelist.net/images/characters/15/315153.jpg"

CHARACTERS = [
    # Wave 1: Fit/Muscular
    {"name": "Yoruichi Shihouin V2", "anime": "Bleach", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Mirko", "anime": "My Hero Academia", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Hibana", "anime": "Fire Force", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Ghislaine Dedoldia", "anime": "Mushoku Tensei", "type": "url", "url": PLACEHOLDER_URL},
    
    # Wave 2: Voluptuous/Big Chested (User Request)
    {"name": "Tier Harribel", "anime": "Bleach", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Miyuki Ayukawa", "anime": "Basquash!", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Michiko Malandro", "anime": "Michiko to Hatchin", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Elena", "anime": "Street Fighter", "type": "url", "url": PLACEHOLDER_URL},
    {"name": "Tsunade", "anime": "Naruto", "type": "url", "url": PLACEHOLDER_URL},
]

SUBJECT_NAME = "Nini"
SUBJECT_DIR = Path("C:/Yuki_Local/Cosplay_Lab/Subjects/Nini")
OUTPUT_DIR = Path("C:/Yuki_Local/Renders_Nini_Black_Waifu")
...
async def process_character(pipeline, session, mal_client, char_info, bypass_lock, semaphore):
    async with semaphore:
        ...
        try:
            ref_path = Path(f"temp_nini_{sanitized_name}.jpg")
            
            if await get_image_source(session, char_info, mal_client, ref_path):
                # FIXED PROMPT: Subject First + Facial Integrity + Body last
                target = (
                    ...
                    "dark skin, voluptuous body, curvy, thick thighs, cleavage, 8k"
                )
...
async def run_batch():
    ...
    print(f"--- Starting Nini (Black Waifu Batch) ---")
