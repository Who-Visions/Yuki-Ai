...
async def extract_facial_ip_v8(client, input_images: list[Path], subject_name: str) -> dict:
    """
    V9: Extract 50-zone Mocap facial geometry from subject photos.
    Enhanced with neck, chin details, dimples, and indentations.
    """
    console.print(f"\n[bold magenta]📐 EXTRACTING 50-ZONE FACIAL IP (Gemini)[/bold magenta]")
...
{{
  ...
  "critical_identity_lock": {{
    ...
    try:
        ...
        console.print(f"   [cyan]Pass 1: thinking=LOW (fast extraction)...[/cyan]")
        ...
        console.print(f"   [cyan]Pass 2: thinking=HIGH (deep refinement)...[/cyan]")
...
class YukiV9Generator:
    """V9 Generator: V8 + Cloud Vision Integration"""
    ...
    async def run(self, gens_per_char: int = 2):
        ...
        # ═══ V8 Step 2: Gemini 18-Zone Extraction ═══
        self.gemini_ip = await extract_facial_ip_v8(
            self.flash_client,
            self.input_images,
            self.subject_name
        )
    ...
    async def _generate_image(self, char: dict, gen_num: int, facial_lock: str) -> tuple[bool, float]:
        ...
        filename = f"{self.subject_name}_{safe_name}_v9_2pass_gen{gen_num}_{timestamp}.png"  # v9_2pass = LOW+HIGH 2-pass
...
║  • 18-Zone Gemini Analysis (semantic features)                               ║
║  • Hybrid Facial Lock (Cloud Vision + Gemini)                                ║
╚══════════════════════════════════════════════════════════════════════════════╝
[/bold magenta]
""")
    
    parser = argparse.ArgumentParser(description="Yuki V9 Generator")
